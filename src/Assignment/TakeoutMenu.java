...
public class TakeoutMenu {
    public static void main(String[] args) {
        ...
        do {
            ...
            switch (mealType) {
                ...
                case "1":
                    TakeoutMenu.displayItalianFoodMenu();
                    int[] feedItalianFood = { 5, 7, 4 };
                    double[] priceItalian = { 17.99, 15.99, 12.99 };
                    int opt = TakeoutMenu.foodChoice(feedItalianFood, priceItalian);
                    TakeoutMenu.finalPrice(feedItalianFood, priceItalian, opt, numberOfPeople);
                    isWrongMenuChoice = false;
                    break;

                case "2":
                    TakeoutMenu.displayChineseFoodMenu();
                    int[] feedChineseFood = { 7, 7, 5 };
                    double[] priceChinese = { 18.99, 18.99, 10.99 };
                    opt = TakeoutMenu.foodChoice(feedChineseFood, priceChinese);
                    TakeoutMenu.finalPrice(feedChineseFood, priceChinese, opt, numberOfPeople);
                    isWrongMenuChoice = false;
                    break;

                case "3":
                    TakeoutMenu.displayAmericanFoodMenu();
                    int[] feedAmericanFood = { 8, 5, 10 };
                    double[] priceAmerican = { 21.99, 22.99, 26.99 };
                    opt = TakeoutMenu.foodChoice(feedAmericanFood, priceAmerican);
                    TakeoutMenu.finalPrice(feedAmericanFood, priceAmerican, opt, numberOfPeople);
                    isWrongMenuChoice = false;
                    break;
                default:
    ...
    public static void finalPrice(int[] feeds, double[] prices, int choice, int guestCount) {
        int trays = TakeoutMenu.determineTrays(guestCount, feeds[choice - 1]);
        double subTotal = TakeoutMenu.getSubtotal(prices[choice - 1], trays);
        double tax = TakeoutMenu.getTax(subTotal, .07);
        double tip = TakeoutMenu.getTip(subTotal, .15);
        double grandTotal = TakeoutMenu.getGrandTotal(subTotal, tax, tip);
        ...
        System.out.printf("Price per person: $ %.2f\n", TakeoutMenu.pricePerPerson(grandTotal, guestCount));
        System.out.println("Left-over servings for the delivery person: "
                + TakeoutMenu.determineLeftOvers(feeds[choice - 1], trays, guestCount));
    }
}
